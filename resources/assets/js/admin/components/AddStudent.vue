                ...
                <div class="panel-body">
                    ...
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        <tr v-if="students.length" v-for="(student,index) in students">
                            ...
                            <td> {{ student.email }}</td>
                            <td> {{ student.phone }}</td>
                            <td class="text-center" @click="deleteStudent(index)"><i title="Delete Student" class="fa fa-trash delete"></i>
                            <td class="text-center" @click="addModerator(index)"><i class="fa fa-user delete" title="Change to moderator"></i>
                            </td>
                        </tr>
                        <tr v-if="initialLoad">
                            <td class="text-center" colspan="8"><i class="fa fa-cog fa-spin fa-2x"></i></td>
                        </tr>
        ...
        methods: {
            ...
            addModerator(index) {
                swal({ // confirmation
                    ...
                    if (result.value) {
                        this.axios.submit('get', 'admin/changeStatus', {'id': this.students[index].id})
                            .then(({status, message}) => {
                                if (status) {
                                    this.students.splice(index, 1);
                                    this.flashNotification('Success!', message, 'success');
